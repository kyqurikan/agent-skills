...
class ExecutiveSummaryTests(unittest.TestCase):
    ...
    def test_clean_model_content_retains_unverified_endpoint_claims(self):
        endpoint_content = (
            "Opening.\n\n"
            "1. **Decision:**\n"
            "   - This endpoint-generated claim is intentionally not fact-checked.\n\n"
            "Closing."
        )
        self.assertEqual(
            module._clean_model_content(endpoint_content),
            endpoint_content,
        )
        forced = module.GeneratedContent(
            endpoint_content,
            (module.OracleTechnology("Unverified Oracle Product", "both"),),
            ("Unverified Non-Oracle Product",),
        )
        rendered = module.render_technology_section(forced, "\n")
        self.assertIn(
            "- Unverified Oracle Product — Customer in use; Oracle team pitched",
            rendered,
        )
        self.assertIn(
            "- Unverified Non-Oracle Product — Customer in use",
            rendered,
        )
    ...
    def test_destination_collision_prevents_generation_and_preserves_both_notes(self):
    ...
    def test_move_failure_rolls_back_review_copy_and_preserves_source(self):
        ...
        with tempfile.TemporaryDirectory(dir="/private/tmp") as temporary:
            ...
            def fail_source_unlink(path, *args, **kwargs):
                if path == note:
                    raise OSError("Synthetic source removal failure.")
            ...
            self.assertIn("could not be moved safely", errors.getvalue())
