#!/usr/bin/env python3
"""Insert a forced executive summary and stage the note for human review."""
...
def preflight_processed_destination(path: Path) -> Path:
    ...
    review_directory = destination.parent
    try:
        info = os.lstat(review_directory)
    ...
    if not stat.S_ISDIR(info.st_mode) or info.st_uid != os.getuid():
        raise SummaryError(
            "The AI Processed path must be a real, user-owned directory."
        )
...
def atomic_write_and_move(
    ...
    """Publish at the review destination, then remove the unchanged source."""
    if destination == path:
    ...
    try:
        try:
            os.mkdir(review_directory)
            created_directory = True
            _fsync_directory(review_directory.parent)
        except FileExistsError:
            pass

        directory_info = os.lstat(review_directory)
        if (
        ...
        # Publish exclusively so a destination created after preflight is not
        # overwritten. The temporary and destination names share an inode.
        ...
        temporary_path.unlink()
        temporary_path = None
        _fsync_directory(review_directory)

        if path.read_bytes() != original:
            raise SummaryError(
                "The note changed while the summary was generated; no move occurred."
            )
        path.unlink()
        source_removed = True
        ...
        failure = SummaryError(
            "The processed note could not be moved safely to AI Processed."
        )
        failure_cause = exc
    ...
    if temporary_path is not None:
        try:
            ...
            cleanup_issues.append(str(temporary_path))
    if destination_created and not source_removed:
        try:
            ...
            cleanup_issues.append(str(destination))
        else:
            ...
            if destination_identity != published_identity:
                cleanup_issues.append(
                    f"{destination} changed after publication and was not removed"
                )
            else:
                try:
                    destination.unlink()
                    ...
                    cleanup_issues.append(str(destination))
    if created_directory and not source_removed:
        try:
            review_directory.rmdir()
            _fsync_directory(review_directory.parent)
        except OSError:
            # A concurrent file or unrecoverable review copy keeps the directory
            # non-empty; any specific rollback artifact is reported above.
            pass

    if failure is None and not cleanup_issues:
        return
    if failure is None:
        failure = SummaryError("The processed note move did not finish cleanly.")
    if cleanup_issues:
        artifacts = ", ".join(cleanup_issues)
        raise SummaryError(
            f"{failure} Rollback was incomplete for: {artifacts}. Inspect both "
            f"the original path ({path}) and review path ({destination})."
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Insert a forced executive summary and stage the note for human review."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Write the summary and reference H2 structure, then move the note "
            "to AI Processed"
        ),
    )
    parser.add_argument(
